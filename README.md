# AnneMC / LeoAi UI

A premium, dark, research-workspace UI inspired by the information hierarchy of modern AI notebook products, with an original LeoAi visual language.

## Preview

Open `preview.svg` to see the static design concept. The working UI is `index.html` with `styles.css` and `app.js`.

## Run

No build step is required for the visual prototype. Open `index.html` in a browser, or serve the folder with any static HTTP server.

## Included

- Three-panel research workspace
- Projects / Sources / Models / Memory / Librarian navigation
- Notebook and source chips
- AI synthesis response card
- Sources / Brain / Agents tabs
- Local engine and GPU status surfaces
- Floating AI composer
- Responsive mobile layout
- Lightweight interaction hooks ready for LeoAi/Ollama API wiring

The interface is intentionally inspired by the calm, document-first workflow of AI research notebooks without copying a proprietary UI one-for-one.
